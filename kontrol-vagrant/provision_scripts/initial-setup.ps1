...
if (-Not (Get-Partition -DriveLetter $diskLetter -ErrorAction SilentlyContinue | Get-Volume | Select-Object -ExpandProperty FileSystemType) -eq "NTFS")
{
    New-Partition -DriveLetter $diskLetter -UseMaximumSize -DiskNumber 1
    Format-Volume -DriveLetter $diskLetter -FileSystem NTFS -NewFileSystemLabel "data" -Confirm:$false
} else
{
